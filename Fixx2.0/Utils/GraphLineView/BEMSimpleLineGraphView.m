...
#pragma clang diagnostic pop
    ...
    if (numberOfPoints == 0) {
        ...
        self.noDataLabel.text = @"No records available at this moment.";
...
- (void)layoutTouchReport {
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        self.panView = [[UIView alloc] initWithFrame:CGRectMake(10, 0, self.viewForBaselineLayout.frame.size.width, self.viewForBaselineLayout.frame.size.height)];
        ...
        if (self.enablePopUpReport == YES && self.alwaysDisplayPopUpLabels == NO) {
            self.popUpLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 50, 15)];
            ...
            self.popUpLabel.font = [UIFont fontWithName:ProximaNovaRegular size:10];
            self.popUpLabel.backgroundColor = [UIColor clearColor];
           // [self.popUpLabel sizeToFit];
            self.popUpLabel.alpha = 0;
            
            self.popUpView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.popUpLabel.frame.size.width + 7, self.popUpLabel.frame.size.height + 4)];
...
//    UILabel *placeholder = [[UILabel alloc] initWithFrame:CGRectMake(0, self.frame.size.height-18, self.frame.size.width, 20)];
//    placeholder.font = self.labelFont;
//    placeholder.textAlignment = 0;
//    placeholder.textColor = BlueColor;
...
#pragma clang diagnostic pop
        ...
        firstLabel.textColor = BlueColor;
...
#pragma clang diagnostic pop
                ...
                labelXAxis.textColor = BlueColor;
                ...
                [labelXAxis setCenter:CGPointMake(((self.viewForBaselineLayout.frame.size.width - self.YAxisLabelXOffset) / (numberOfPoints-1)) * (i*numberOfGaps - 1 - offset) + self.YAxisLabelXOffset-27, self.frame.size.height - lRect.size.height/2)];
...
- (void)drawYAxis {
    ...
    if (self.autoScaleYAxis) {
        ...
        for (NSNumber *dotValue in dotValues) {
            ...
            UILabel *labelYAxis = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.YAxisLabelXOffset, 10)];
            //labelYAxis.text = [NSString stringWithFormat:@"%.2f",dotValue.floatValue];
            labelYAxis.textAlignment = NSTextAlignmentRight;
            labelYAxis.font = self.labelFont;
            labelYAxis.textColor = BlueColor;
...
- (void)displayPermanentLabelForPoint:(BEMCircle *)circleDot {
   ...
   // permanentPopUpLabel.text = [NSString stringWithFormat:@"%@", [NSNumber numberWithInteger:circleDot.absoluteValue]];
    ...
    NSString *numberString = [formatter stringFromNumber:[NSNumber numberWithFloat:circleDot.absoluteValue]];
    
    permanentPopUpLabel.text = numberString;
    ...
    [permanentPopUpLabel setTextColor:BlueColor];
    ...
    permanentPopUpView.layer.borderColor = BlueColor_Light.CGColor;
    permanentPopUpView.layer.borderWidth = 1.0;
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint {
    ...
    self.popUpView.layer.borderColor = BlueColor_Light.CGColor;
    ...
    if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
        ...
        self.popUpLabel.text = [NSString stringWithFormat:@"%.2f", (float)[[dataPoints objectAtIndex:((float)closestDot.tag - 100)] floatValue]];
    ...
    self.popUpLabel.center = self.popUpView.center;
    [self.popUpLabel setFont:[UIFont fontWithName:@"Arial" size:11.0f]];
}
