  ...
  ePieChartDataModel:(EPieChartDataModel *)ePieChartDataModel
{
    ...
    if (self)
    {
        //self.clipsToBounds = YES;
        /** Default settings*/
        _budgetColor = [UIColor grayColor];
        _currentColor = EYellow;
        _estimateColor = [UIColor lightGrayColor];
        _lineWidth = radius / 6;
        ...
        UIImageView *imgExpense = [[UIImageView alloc] init];
        imgExpense.image = [UIImage imageNamed:@"cup-icon.png"];
        imgExpense.frame = CGRectMake(40, 30, 75, 75);
        [_contentView addSubview:imgExpense];
        
        _fixxValueLabel = [[UICountingLabel alloc] initWithFrame:self.frame];
        //budgetLabel.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds) * 1);
        _fixxValueLabel.frame = CGRectMake(40, 105, 80, 30);
        _fixxValueLabel.textAlignment = NSTextAlignmentCenter;
        _fixxValueLabel.method = UILabelCountingMethodEaseInOut;
        _fixxValueLabel.font = [UIFont fontWithName:ProximaNovaBold size:20];
        _fixxValueLabel.textColor = [UIColor whiteColor];
        [_fixxValueLabel setBackgroundColor:[UIColor clearColor]];
        _fixxValueLabel.format = @"$%.1f";
        [_contentView addSubview:_fixxValueLabel];
        [_fixxValueLabel countFrom:0 to:_ePieChartDataModel.budget withDuration:2.0f];
...
- (void) reloadContent
{
    ...
    UIBezierPath* circleEstimatePath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds))
                                                                      radius: _radius * 0.8
                                                                  startAngle: M_PI_2 * 3 - (_ePieChartDataModel.current / _ePieChartDataModel.budget) * (M_PI * 2)
