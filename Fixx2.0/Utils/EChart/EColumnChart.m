...
@property (strong, nonatomic) NSMutableDictionary *eColumnsIncome;
...
@synthesize eColumnsIncome = _eColumnsIncome;
...
- (void)setDataSource:(id<EColumnChartDataSource>)dataSource
{
    if (_dataSource != dataSource)
    {
        ...
        if (![_dataSource respondsToSelector:@selector(highestValueEColumnChartIncome:)])
        {
            NSLog(@"@selector(highestValueEColumnChart:) Not Implemented!");
            return;
        }
        
        if (![_dataSource respondsToSelector:@selector(eColumnChart:valueForIndex:forColumn:)])
        {
...
//                    [self addSubview:eColumnChartLabel];
            ...
            else
            {
                ...
                for (int i = 0; i < 11; i++)
                {
//                    UIView *horizontalLine = [[UIView alloc] initWithFrame:CGRectMake(0, heightGap * i, self.frame.size.width, 5)];
//                    horizontalLine.backgroundColor = ELightGrey;
//                    [self addSubview:horizontalLine];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _eColumnsIncome = [NSMutableDictionary dictionary];
        [self initData];
    }
    return self;
}
...
- (void)reloadData
{
    ...
    float widthOfTheColumnShouldBe = self.frame.size.width / (float)(totalColumnsRequired + (totalColumnsRequired + 1) * 4);
    ...
    //Set Green Cloumn Bar
    NSInteger GreenColumnSpace =0;
    
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        NSInteger currentIndex = _leftMostIndex - i * DIRECTION;
       // EColumnDataModel *eColumnDataModel = [_dataSource eColumnChart:self valueForIndex:currentIndex forColumn:@"Expense"];
        ...
        if (nil == eColumn)
        {
            eColumn = [[EColumn alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 3.5)+GreenColumnSpace, 0, widthOfTheColumnShouldBe, self.frame.size.height)];
        ...
        {
            eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 3.5), self.frame.size.height, widthOfTheColumnShouldBe, 20)];
            [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
        ...
        GreenColumnSpace = GreenColumnSpace +8;
    }
    
    //Set Pink Cloumn Bar
    NSInteger ColumnSpace =0;
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        NSInteger currentIndex = _leftMostIndex - i * DIRECTION;
        // EColumnDataModel *eColumnDataModel = [_dataSource eColumnChart:self valueForIndex:currentIndex forColumn:@"Expense"];
        
        EColumnDataModel *eColumnDataModel1 = [_dataSource eColumnChart:self valueForIndex:currentIndex forColumn:@"Income"];
        
        if (eColumnDataModel1 == nil)
            eColumnDataModel1 = [[EColumnDataModel alloc] init];
        
        /** Judge which is the max value and which is min, then set color correspondingly */
        if (eColumnDataModel1.value > maxValue) {
            maxIndex =  currentIndex;
            maxValue = eColumnDataModel1.value;
        }
        
        if (eColumnDataModel1.value < minValue) {
            minIndex = currentIndex;
            minValue = eColumnDataModel1.value;
        }
        
        /** Construct Columns*/
        EColumn *eColumn1 = [_eColumnsIncome objectForKey: [NSNumber numberWithInteger:currentIndex]];
        if (nil == eColumn1)
        {
            eColumn1 = [[EColumn alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 3.5)+widthOfTheColumnShouldBe+ColumnSpace, 0, widthOfTheColumnShouldBe, self.frame.size.height)];
            //eColumn.barColor = EGrey;
            eColumn1.barColor = _maxColumnColor;
            eColumn1.backgroundColor = [UIColor clearColor];
            eColumn1.grade = eColumnDataModel1.value / _fullValueOfTheGraph;
            eColumn1.eColumnDataModel = eColumnDataModel1;
            eColumn1.tag = 102;
            [eColumn1 setDelegate:self];
            [self addSubview:eColumn1];
            [_eColumnsIncome setObject:eColumn1 forKey:[NSNumber numberWithInteger:currentIndex ]];
        }

        eColumn1.barColor = _maxColumnColor;
        ColumnSpace = ColumnSpace + 8;
...
- (void)moveLeft
{
    ...
    EColumnDataModel *eColumnDataModel = [_dataSource eColumnChart:self valueForIndex:index forColumn:@""];
